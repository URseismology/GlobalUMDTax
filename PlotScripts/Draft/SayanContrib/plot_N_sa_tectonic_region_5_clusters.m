...
for iSta = 1 : length(sequencedData.station)
    ...
    switch cluster
        ...
        case 5
            m_plot(lon, lat, 'marker','^', 'color','k','linewi',1,...
                'linest','none','markersize',10,'markerfacecolor',[0.333 0.365 0.922]);
            m_text(lon,lat,sta,'Fontsize',7, 'FontWeight','bold');
    end
    hold on
end
...
colors = {[0.957 0 0], [0.988 0.996 0], [0.678 0 0.867], [0.341 0.086 0.078], [0.333 0.365 0.922]};
ax2=axes('position',[0.65,0.15,0.17,0.1],'Color','None','Visible','off');
rectangle('position',[0,-0.25,1,1.5],'FaceColor','white');
...
color_tec_regions = {[0.875 0.741 0.145],[0.992 1 0.576],[0.659 0.125 0.114],[0.722, 0.843, 0.447]};
...
yticks;
yticklabels({'\bf \color[rgb]{0.957 0 0} N1','\bf \color[rgb]{0.988 0.996 0} N2','\bf \color[rgb]{0.678 0 0.867} N3','\bf \color[rgb]{0.341 0.086 0.078} N4','\bf \color[rgb]{0.333 0.365 0.922} N5'});
rectangle('Position', [x, y, width, height], 'FaceColor', colors{i});
camroll(270)
...
bh(1).FaceColor=[0.957 0 0];bh(2).FaceColor=[0.988 0.996 0];bh(3).FaceColor=[0.678 0 0.867];bh(4).FaceColor=[0.341 0.086 0.078];bh(5).FaceColor=[0.333 0.365 0.922];
xlim([0 20])
legend({'N1','N2','N3','N4','N5'}, 'Location','best','FontWeight','normal');
...
colormap([[0.875 0.741 0.145]; [0.992 1 0.576]; [0.659 0.125 0.114]; [0.722, 0.843, 0.447]]);
cb_bar = colorbar;
