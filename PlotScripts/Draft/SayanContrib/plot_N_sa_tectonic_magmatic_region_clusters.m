...
mag_3 = sa_andean_magnetic_arc(strcmp(sa_andean_magnetic_arc.Region,'Active_Magnetic_Arc_3'),:);
mag_4 = sa_andean_magnetic_arc(strcmp(sa_andean_magnetic_arc.Region,'Active_Magnetic_Arc_4'),:);
fore_arc = sa_andean_magnetic_arc(strcmp(sa_andean_magnetic_arc.Region,'Forearc'),:);
retro_arc = sa_andean_magnetic_arc(strcmp(sa_andean_magnetic_arc.Region,'RetroArc'),:);


all_mag = [mag_1; mag_2; mag_3; mag_4; fore_arc; retro_arc];

all_mag(all_mag.Region=="Active_Magnetic_Arc_1","Region_Grouped") = {"Active Magnetic Arc"};
all_mag(all_mag.Region=="Active_Magnetic_Arc_2","Region_Grouped") = {"Active Magnetic Arc"};
all_mag(all_mag.Region=="Active_Magnetic_Arc_3","Region_Grouped") = {"Active Magnetic Arc"};
all_mag(all_mag.Region=="Active_Magnetic_Arc_4","Region_Grouped") = {"Active Magnetic Arc"};
all_mag(all_mag.Region=="Forearc","Region_Grouped") = {"Forearc"};
all_mag(all_mag.Region=="RetroArc","Region_Grouped") = {"RetroArc"};


unq_mag_region_grp = unique(all_mag.Region_Grouped);
tot_mag_region_grp = length(unq_mag_region_grp);

total_clusters = length(unique(all_clusters));
mag_cluster_cnt = zeros(tot_mag_region_grp,total_clusters);

for j=1:tot_mag_region_grp
    T = all_mag(strcmp(all_mag.Region_Grouped, unq_mag_region_grp{j}),:);
    all_unq_mag_regions = unique(T.Region);
    num_unq_mag_regions = size(all_unq_mag_regions,1);
    mag_is_inside = zeros(length(all_stations),1);
    for i=1:num_unq_mag_regions
        T_sub = T(strcmp(T.Region, all_unq_mag_regions{i}),:);
        geo_shape = polyshape(T_sub.Latitude,T_sub.Longitude);
        is_inside_temp = isinterior(geo_shape,station_coord(:,1),station_coord(:,2));
        mag_is_inside = is_inside_temp + mag_is_inside;
    end
    matching_clusters_mag_idx = find(mag_is_inside>=1);
    matching_clusters_mag = all_clusters(matching_clusters_mag_idx);
    [clus_cnt_mag,clus_mag] = groupcounts(matching_clusters_mag);
    mag_cluster_cnt(j,clus_mag) = clus_cnt_mag;
end
...
%m_hatch(amazonian_craton1.Longitude,amazonian_craton1.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(amazonian_craton1.Longitude,amazonian_craton1.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(amazonian_craton2.Longitude,amazonian_craton2.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(amazonian_craton2.Longitude,amazonian_craton2.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(sm_craton_1.Longitude,sm_craton_1.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(sm_craton_1.Longitude,sm_craton_1.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(sm_craton_2.Longitude,sm_craton_2.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(sm_craton_2.Longitude,sm_craton_2.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(sm_craton_3.Longitude,sm_craton_3.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(sm_craton_3.Longitude,sm_craton_3.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(sm_craton_4.Longitude,sm_craton_4.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(sm_craton_4.Longitude,sm_craton_4.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(sm_craton_5.Longitude,sm_craton_5.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(sm_craton_5.Longitude,sm_craton_5.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(soafrans_craton.Longitude,soafrans_craton.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(soafrans_craton.Longitude,soafrans_craton.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(lv_craton.Longitude,lv_craton.Latitude, 'single',1,0.01,'color',[0.012, 0.576, 0.871 0.5]);
m_line(lv_craton.Longitude,lv_craton.Latitude, 'color',[0 0 0],'linewi',2);

%m_hatch(andean_region.Longitude,andean_region.Latitude, 'single',1,0.01,'color',[0.941, 0.608, 0.624 0.5]);
m_line(andean_region.Longitude,andean_region.Latitude, 'color',[0 0 0],'linewi',2);

%m_hatch(big_basin.Longitude,big_basin.Latitude, 'single',1,0.01,'color',[0.729, 0.702, 0.839 0.5]);
m_line(big_basin.Longitude,big_basin.Latitude, 'color',[0 0 0],'linewi',2);

%m_hatch(proto_1.Longitude,proto_1.Latitude, 'single',1,0.01,'color',[0.463, 0.78, 0.941 0.2]);
m_line(proto_1.Longitude,proto_1.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(proto_2.Longitude,proto_2.Latitude, 'single',1,0.01,'color',[0.463, 0.78, 0.941 0.2]);
m_line(proto_2.Longitude,proto_2.Latitude, 'color',[0 0 0],'linewi',2);
%m_hatch(proto_3.Longitude,proto_3.Latitude, 'single',1,0.01,'color',[0.463, 0.78, 0.941 0.2]);
m_line(proto_3.Longitude,proto_3.Latitude, 'color',[0 0 0],'linewi',2);

m_hatch(mag_1.Longitude,mag_1.Latitude, 'single',1,0.01,'color',[0.945, 0.298, 0.31 0.2]);
m_line(mag_1.Longitude,mag_1.Latitude, 'color',[0 0 0],'linewi',1);
m_hatch(mag_2.Longitude,mag_2.Latitude, 'single',1,0.01,'color',[0.945, 0.298, 0.31 0.2]);
m_line(mag_2.Longitude,mag_2.Latitude, 'color',[0 0 0],'linewi',1);
m_hatch(mag_3.Longitude,mag_3.Latitude, 'single',1,0.01,'color',[0.945, 0.298, 0.31 0.2]);
m_line(mag_3.Longitude,mag_3.Latitude, 'color',[0 0 0],'linewi',1);
m_hatch(mag_4.Longitude,mag_4.Latitude, 'single',1,0.01,'color',[0.945, 0.298, 0.31 0.2]);
m_line(mag_4.Longitude,mag_4.Latitude, 'color',[0 0 0],'linewi',1);

m_hatch(fore_arc.Longitude,fore_arc.Latitude, 'single',1,0.01,'color',[0.882, 0.698, 0.824 0.2]);
m_line(fore_arc.Longitude,fore_arc.Latitude, 'color',[0 0 0],'linewi',1);

m_hatch(retro_arc.Longitude,retro_arc.Latitude, 'single',1,0.01,'color',[0.976, 0.722, 0.451 0.2]);
m_line(retro_arc.Longitude,retro_arc.Latitude, 'color',[0 0 0],'linewi',1);
...
%% Plot Extra Sations
% for i=1:size(new_stns_metadata,1)
%     m_plot([new_stns_metadata(i,:).Lon],[new_stns_metadata(i,:).Lat],'bo');
%     m_text([new_stns_metadata(i,:).Lon],[new_stns_metadata(i,:).Lat],[new_stns_metadata(i,:).Lon], ...
%         [new_stns_metadata(i,:).Station],'Fontsize',7, 'FontWeight','bold');
% end
% hold on

%% Plot Legends & Information Box
totSta = length(sequencedData.clusters);
colors = {[0.957 0 0], [0.988 0.996 0], [0.678 0 0.867], [0.341 0.086 0.078]};
% ax2=axes('position',[0.65,0.15,0.17,0.1],'Color','None','Visible','off');
% rectangle('position',[0,0,1,1],'FaceColor','white');
% for i = 1:size(cluster_cnt,2)
...
ax3=axes('position',[0.65,0.82,0.15,0.085],'Color','None','Visible','off');
rectangle('position',[0,0,1,1],'FaceColor','white');
color_mag_regions = {[0.945, 0.298, 0.31], [0.882, 0.698, 0.824], [0.976, 0.722, 0.451]};
mag_regions = unq_mag_region_grp;
for i = 1:length(mag_regions)
    x = 0.03; y = 1.08 - i * 0.24;   
    width = 0.1; height = 0.08;   
    rectangle('Position', [x, y, width, height], 'FaceColor', color_mag_regions{i});
    text(x + width + 0.01, y + height / 2, mag_regions{i}, 'VerticalAlignment', 'middle','Fontsize',7, 'FontWeight','bold');
end
hold on
...
yticks;
yticklabels({'\bf \color[rgb]{0.957 0 0} N1','\bf \color[rgb]{0.988 0.996 0} N2','\bf \color[rgb]{0.678 0 0.867} N3','\bf \color[rgb]{0.341 0.086 0.078} N4'});
rectangle('Position', [x, y, width, height], 'FaceColor', colors{i});
camroll(270)
...
mag_regions = unq_mag_region_grp;
bh = barh(mag_cluster_cnt,'BarWidth',0.9);
bh(1).FaceColor=[0.957 0 0];bh(2).FaceColor=[0.988 0.996 0];bh(3).FaceColor=[0.678 0 0.867];bh(4).FaceColor=[0.341 0.086 0.078];
xlim([0 20])
legend({'N1','N2','N3','N4'}, 'Location','best','FontWeight','normal');
set(gca,'YTick',[], 'YTickLabel',[]);
set(gca,'XTick',[],'XTickLabel',[]);

for i=1:size(mag_cluster_cnt,2)
    bhxtips = bh(i).XEndPoints-0.09;
    bhytips = bh(i).YEndPoints+0.53;
...
colormap([[0.945, 0.298, 0.31]; [0.882, 0.698, 0.824]; [0.976, 0.722, 0.451];]);
cb_bar = colorbar;
cb_bar.Position = [0.11    0.11    0.0138    0.3876];
cb_bar.Ticks=linspace(0.15,0.85,3); cb_bar.TickLabels = mag_regions;
...
fig_finale = gcf;
saveas(fig_finale, ['/scratch/tolugboj_lab/Sayan_Swar_WS/PythonEnv/Python_Notebooks/GoGlobal/scripts/rf_prep_slurm_step_7/srtfista_output/south america/parameter_set_1/' 'SA_Magmatic_Plot.jpg']);
