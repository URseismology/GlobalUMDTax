function Figure1_Amap_revised_NA()
    clear; close all; clc;
    addpath('./Data/m_map');
    ...
    % Grid for North America region
    longridinterp = -180:0.25:180;
    latgridinterp = -90:0.25:90;
    ...
    % Filter stations to US Lower 48 bounds
    na_idx = (st_lon >= -125 & st_lon <= -70 & st_lat >= 24 & st_lat <= 50 & ~isnan(st_depth));
    st_lon = st_lon(na_idx);
    st_lat = st_lat(na_idx);
    st_c = st_c(na_idx);

    % Generate both versions of the plot
    generate_plot(st_lon, st_lat, st_c, xq, yq, Age_Discrete, custom_cmap, false, 'Figures/Global_Study/Figure1_Amap_revised_NA_v1.png');
    generate_plot(st_lon, st_lat, st_c, xq, yq, Age_Discrete, custom_cmap, true, 'Figures/Global_Study/Figure1_Amap_revised_NA_v2.png');
end

function generate_plot(st_lon, st_lat, st_c, xq, yq, Age_Discrete, custom_cmap, thin_flag, output_filename)
    f = figure('Name', 'Figure 1: NA Age-Craton Map', 'Position', [50, 100, 1600, 700], 'Visible', 'off');
    ...
    if thin_flag
        disp('Applying station thinning (within 1 degree) for NA...');
    ...
    psz = 21;
    na_proj_limits_lat = [24 50];
    na_proj_limits_lon = [-125 -70];

    %% PANEL A: C1, C2, C3
    ax_a = axes('Position', [0.03, 0.18, 0.44, 0.78]);
    m_proj('miller', 'lat', na_proj_limits_lat, 'lon', na_proj_limits_lon);
    hold on;
    ...
    plot_plate_boundaries(na_proj_limits_lat, na_proj_limits_lon);
...
function plot_plate_boundaries(lat_lim, lon_lim)
