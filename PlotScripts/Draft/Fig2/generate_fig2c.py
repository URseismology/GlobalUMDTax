...
with open('Figure2b_Maps_Stats.m', 'r') as f:
    code = f.read()

# Extract preamble (imports and grid loading)
preamble = re.search(r'function Figure2b_Maps_Stats\(\).*?%% 4. Load Clustering Statistics', code, re.DOTALL).group(0)
preamble = preamble.replace('Figure2b_Maps_Stats', 'Figure2c_MapsLocs')
...
with open('Figure2c_MapsLocs.m', 'w') as f:
    f.write(new_code)
